...
def __dense_block(x, nb_layers, nb_filter, growth_rate, bottleneck=False, dropout_rate=None, weight_decay=1E-4,
    ...
    x_list = [x]
    if return_concat_list==True:
        deb.prints(nb_layers)
...
def __create_fcn_dense_net(nb_classes, img_input, include_top, nb_dense_block=5, growth_rate=12,
    ...
    for block_idx in range(nb_dense_block):
        ...
        #if len(concat_list[1:])>1:
        #    l = concatenate([m for m in concat_list[1:]], axis=concat_axis)
        #else:
        #    l = concat_list[1:]
        print("Doing the l concat")
        deb.prints(len(concat_list))
        deb.prints(K.int_shape(concat_list[0]))
        deb.prints(K.int_shape(concat_list[1]))
        deb.prints(K.int_shape(concat_list[2]))
        l = concatenate([m for m in concat_list[1:]], axis=concat_axis)
        deb.prints(K.int_shape(l))
        #l = concatenate([m for m in concat_list], axis=concat_axis)
        #l = concat_list[1:]
...
if __name__ == '__main__':
    model = DenseNetFCNTimeDistributed((14,32, 32, 2), nb_dense_block=2, growth_rate=64, dropout_rate=0.2,
